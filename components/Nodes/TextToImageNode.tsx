            ...
            <div className="absolute top-full left-1/2 -translate-x-1/2 w-full min-w-[450px] pt-3 z-[70] pointer-events-auto" onMouseDown={(e) => e.stopPropagation()} onTouchStart={(e) => e.stopPropagation()} data-interactive="true">
                 ...
                 <div className={`${controlPanelBg} rounded-2xl p-3 shadow-2xl flex flex-col gap-3 border`}>
                      <div className="flex flex-col" data-interactive="true">
                          ...
                          {/* Image/Video Token Insertion Buttons */}
                          {inputs.length > 0 && (
                                  ...
                                  {(() => {
                                      ...
                                      return inputs.map((input, i) => {
                                          ...
                                          return (
                                              <button 
                                                  key={i}
                                                  onClick={() => inputRef.current?.insertText(tokenText)}
                                                  onTouchStart={(e) => e.stopPropagation()}
                                                  onTouchEnd={(e) => { 
                                                      e.preventDefault(); 
                                                      e.stopPropagation(); 
                                                      inputRef.current?.insertText(tokenText);
                                                  }}
                      ...
                      <div className="flex items-center justify-between gap-2 h-7">
                          <LocalCustomDropdown options={imageModels} value={data.model || 'BananaPro'} onChange={(val: any) => updateData(data.id, { model: val })} isOpen={activeDropdown === 'model'} onToggle={() => setActiveDropdown(activeDropdown === 'model' ? null : 'model')} onClose={() => setActiveDropdown(null)} align="left" width="w-[130px]" isDark={isDark} />
