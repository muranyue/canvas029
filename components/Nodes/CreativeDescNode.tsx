    ...
    return (
        <>
          ...
          <div className={`w-full h-full border rounded-xl p-4 flex flex-col shadow-xl ${containerBg} ${containerBorder}`}>
              ...
              <textarea className={`w-full flex-1 border rounded-lg p-3 text-[10px] leading-relaxed resize-none focus:outline-none transition-colors no-scrollbar ${inputBg}`} placeholder="Enter your initial idea here..." value={data.prompt || ''} onChange={(e) => updateData(data.id, { prompt: e.target.value })} onMouseDown={(e) => e.stopPropagation()} onTouchStart={(e) => e.stopPropagation()} onWheel={(e) => e.stopPropagation()} />
              <button onClick={() => onGenerate(data.id)} className="mt-3 w-full bg-cyan-500/10 hover:bg-cyan-500/20 text-cyan-400 text-xs font-bold py-2 rounded-lg flex items-center justify-center gap-2 border border-cyan-500/20 transition-all">
              ...
              {data.optimizedPrompt && isSelectedAndStable && showControls && (
                  <div className={`absolute top-full left-0 w-full mt-3 border rounded-xl p-4 text-xs z-[70] shadow-2xl animate-in slide-in-from-top-2 duration-200 ${controlPanelBg} ${isDark ? 'text-gray-300' : 'text-gray-700'}`} onTouchStart={(e) => e.stopPropagation()}>
