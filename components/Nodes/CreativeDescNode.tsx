    ...
    return (
        <>
          ...
          <div className={`w-full h-full border rounded-xl p-4 flex flex-col shadow-xl ${containerBg} ${containerBorder}`}>
              ...
              <textarea className={`w-full flex-1 border rounded-lg p-3 text-[10px] leading-relaxed resize-none focus:outline-none transition-colors no-scrollbar ${inputBg}`} placeholder="Enter your initial idea here..." value={data.prompt || ''} onChange={(e) => updateData(data.id, { prompt: e.target.value })} onMouseDown={(e) => e.stopPropagation()} onWheel={(e) => e.stopPropagation()} data-interactive="true" />
