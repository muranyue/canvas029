...
export const CreativeDescNode: React.FC<CreativeDescNodeProps> = ({
    ...
    return (
        <>
          <div 
            className="absolute bottom-full left-0 w-full mb-2 flex items-center justify-between pointer-events-auto"
          ...
          <div className={`w-full h-full border rounded-xl p-4 flex flex-col shadow-xl ${containerBg} ${containerBorder}`}>
              <div className={`flex items-center gap-2 text-xs mb-3 ${isDark ? 'text-zinc-400' : 'text-gray-500'}`}>
                  <div className="p-1.5 bg-cyan-500/10 rounded-md"><Icons.Wand2 size={14} className="text-cyan-400"/></div>
                  <span className="uppercase font-bold tracking-wider text-[10px]">Creative Assistant</span>
              </div>
              <textarea 
              ...
              <button 
                  ...
                  onMouseDown={(e) => e.stopPropagation()}
                  onTouchStart={(e) => e.stopPropagation()}
                  className="mt-3 w-full bg-cyan-500/10 hover:bg-cyan-500/20 text-cyan-400 text-xs font-bold py-2 rounded-lg flex items-center justify-center gap-2 border border-cyan-500/20 transition-all"
              >
              ...
              {data.optimizedPrompt && isSelectedAndStable && showControls && (
                  ...
                  >
                      ...
                      <div className="leading-relaxed select-text cursor-text">{data.optimizedPrompt}</div>
                  </div>
              )}
          </div>
        </>
